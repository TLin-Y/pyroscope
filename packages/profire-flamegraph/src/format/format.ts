...
export function getFormatter(
  ...
  switch (unit) {
    case 'samples':
      return new DurationFormatter(max, unitLevel);
    case 'objects':
    ...
    case 'lock_nanoseconds':
      return new DurationFormatter(max, unitLevel, unitStr);
    case 'lock_samples':
      return new BytesFormatter(max, unitLevel);
    case 'trace_samples':
      return new DurationFormatter(max, unitLevel, unitStr);
...
const timeLevels: [number, string][] = [
  ...
  [60 * 60 * 1e9, 'hour'],
  [24 * 60 * 60 * 1e9, 'day'],
  [30.44 * 24 * 60 * 60 * 1e9, 'month'],
  [365.25 * 24 * 60 * 60 * 1e9, 'year'],
];
...
export class DurationFormatter extends Formatter {
  ...
  constructor(maxDur: number, unitLevel: number, units?: string) {
    super();
    this.units = units || '';
    ...
    const f = () => {
      for (let i = timeLevels.length - 1; i >= 0; i--) {
        ...
        if (maxDur >= level[0]) {
          const [defaultDiv, defaultUnitStr] = level;
          this.divider = defaultDiv;
          this.suffix = defaultUnitStr;
          break;
        }
      }
    };
